...
export default function TopNumbers() {
  ...
  const { drawId: routeDrawId } = useLocalSearchParams<{ drawId?: string }>();
  const initialDrawId = Number(routeDrawId) > 0 ? Number(routeDrawId) : "";
  ...
  // Dealer options include every page and follow the selected vendor scope.
  const { data: dealerOptions = [], isError: dealersError, refetch: refetchDealers } = useQuery<Dealer[]>({
    queryKey: ["/draw-booking/top-numbers/dealers/", { vendor: vendorId }],
    queryFn: () => api.get("/draw-booking/top-numbers/dealers/", {
      params: { vendor: vendorId || undefined },
    }).then((res) => res.data),
    enabled: !!user?.superuser,
  });
  const dealers = useMemo(() => dealerOptions.map((dealer) => ({
    ...dealer,
    username: vendorId ? dealer.username : `${dealer.username} (${dealer.vendor_name})`,
  })), [dealerOptions, vendorId]);

  const { data: draws = [] } = useQuery<Draw[]>({
    queryKey: ["/draw/"],
    queryFn: () => api.get("/draw/").then((res) =>
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      await Promise.all([refetch(), refetchDealers()]);
    } finally {
      setRefreshing(false);
    }
  };
        ...
        <View className="px-4 pt-4 pb-2 flex-row items-center justify-between">
          <View className="flex-1 mr-3">
            ...
            <Text className="text-xs text-gray-500 mt-1">
              Counts and amounts · {vendorId ? "Selected vendor" : "All vendors combined"}
            </Text>
          </View>
        ...
        <View className="mx-4 mt-3 mb-4 border border-gray-200 rounded-2xl p-3 bg-gray-50">
          ...
          {dealersError && (
            <Text className="text-xs text-red-600 mt-2">
              Unable to load dealer filters. Pull down to retry.
            </Text>
          )}
          ...
          <View className="mt-3">
            ...
            <Dropdown
              data={[{ label: "All draws", value: "" as const }, ...draws.map((draw) => ({
                label: draw.name,
                value: draw.id,
              }))]}
              labelField="label"
              ...
              onChange={(item) => setDrawId(item.value)}
              style={{
              ...
              itemTextStyle={{ color: "#111827", fontSize: 12 }}
            />
          </View>
