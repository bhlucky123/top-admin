            ...
            renderItem={({ item }) => {
              ...
              return (
                ...
                >
                  ...
                  {active && <AntDesign name="check" size={18} color="#ea580c" />}
                </TouchableOpacity>
              );
            }}
            ListEmptyComponent={
  ...
  return (
    ...
    >
      ...
      <View className="flex-row items-center">
        ...
        <ChevronDown size={14} color="#ea580c" style={{ marginLeft: 6 }} />
      </View>
    </TouchableOpacity>
  );
}
...
function TableRow({ item, even }: { item: RecallLog; even: boolean }) {
  return (
    <View
      style={[
        tableStyles.row,
...
export default function RecallLogScreen() {
  ...
  let totalRecalled = 0;
  for (const i of items) totalRecalled += i?.recalled_count ?? 0;
  ...
  const typeCounts = useMemo(() => {
    ...
    for (const i of items) map[i.type] = (map[i.type] || 0) + i.recalled_count;
    return map;
  }, [items]);
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white border-b border-gray-200 px-6 pt-14 pb-4">
        <View className="flex-row items-center justify-between">
          ...
          <Text className="text-xl font-bold text-gray-800">Recall Log</Text>
      ...
      <View className="bg-white px-6 pt-4 pb-5 border-b border-gray-100 gap-2">
        {drawId ? (
          <View className="flex-row items-center bg-orange-50 border border-orange-100 rounded-xl px-3 py-2.5">
            <Ticket size={14} color="#c2410c" />
            <Text className="text-[10px] text-orange-500 font-semibold uppercase ml-2 mr-1">
              Draw
            </Text>
            <Text
              className="text-orange-800 font-bold text-sm flex-1"
              numberOfLines={1}
            >
              {drawName}
            </Text>
        ...
        >
          ...
          <Text
            className={`text-sm font-medium ${
              todayOnly ? "text-orange-700" : "text-gray-800"
            }`}
          >
        ...
        {!isLoading && (
          <>
            <View className="flex-row gap-2 mt-1">
              ...
              <View className="px-3 py-1.5 rounded-lg bg-orange-50">
              ...
              <>
                <View className="flex-row gap-2 mt-2">
                  {(Object.keys(typeCounts) as MonitoringType[]).map((t) => (
                    <View
                      key={t}
                      className="flex-1 bg-orange-50 rounded-lg px-3 py-2 items-center"
                    >
                ...
                <View className="flex-row flex-wrap gap-2 mt-1">
                  {(
                    Object.entries(subTypeCounts) as [
                      MonitoringSubType,
                      number
                    ][]
                  ).map(([st, count]) => (
      ...
      {isError ? (
        <View className="flex-1 justify-center items-center px-8">
          <View className="bg-red-50 border border-red-200 rounded-2xl px-6 py-8 items-center w-full">
            ...
            <TouchableOpacity
              onPress={() => refetch()}
              className="bg-orange-600 px-8 py-3 rounded-xl mt-4"
            >
